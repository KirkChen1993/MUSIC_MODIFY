...
protected:
	...
	inline std::complex<double> eval_constr( int num, size_t icon, double kx, double ky, double kz )
	{
		double re, im, kdotx, k2;
//		double OmegaH2=3.0*2*1364.01684;
		double OmegaH2=1.0;
		...
		switch(num)
		{
            case 0 : re  = im = exp(-k2*cset_[icon].gRg2/2.0); break;
            case 1 : re  = im = kx*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 2 : re  = im = ky*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 3 : re  = im = kz*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 4 : re  = im = -kx*kx*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 5 : re  = im = -ky*ky*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 6 : re  = im = -kz*kz*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 7 : re  = im = -kx*ky*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 8 : re  = im = -kx*kz*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 9 : re  = im = -ky*kz*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 10 : re  = im = OmegaH2*kx/k2*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 11 : re  = im = OmegaH2*ky/k2*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 12 : re  = im = OmegaH2*kz/k2*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 13 : re  = im = OmegaH2*(kx*kx/k2-1.0/3)*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 14 : re  = im = OmegaH2*(ky*ky/k2-1.0/3)*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 15 : re  = im = OmegaH2*(kx*ky/k2)*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 16 : re  = im = OmegaH2*(kx*kz/k2)*exp(-k2*cset_[icon].gRg2/2.0); break;
            case 17 : re  = im = OmegaH2*(ky*kz/k2)*exp(-k2*cset_[icon].gRg2/2.0);
		}

		if ((num>=10 && num <=12) or (num>=1 && num<=3))
		{
			re *= -sin( kdotx );
			im *= cos( kdotx );
		}
	...
	//! test constraints
	void constraint_test( double dx, fftwf_complex* cw, size_t nx, size_t ny, size_t nz, std::vector<double>& h0 );
	...
	//! test constraints
	void constraint_test( double dx, fftw_complex* cw, size_t nx, size_t ny, size_t nz, std::vector<double>& h0 );
#endif
...
//		unsigned levelmax = pcf_->getValue<unsigned>("setup","levelmax");
		...
		if( bperiodic )
		{
			...
			constraint_test(dx, cw, nx, ny, nz, h0);
