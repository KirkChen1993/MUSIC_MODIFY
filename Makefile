...
CPATHS  = -I. -I/users/jchen/libraries/include/ -I/users/jchen/libraries/include/fftw3/include -I/opt/local/include -I/usr/local/include 
LPATHS  = -L/users/jchen/libraries/lib -L/users/jchen/libraries/fftw3/lib -L/opt/local/lib -L/usr/local/lib 
...
$(TARGET): $(OBJS) plugins/nyx_plugin/*.cpp
	...
	$(CC) $(LPATHS) -o $@ $^ $(LFLAGS) $(BLOBJS) -lifcore -no-pie
else
$(TARGET): $(OBJS)
	$(CC) $(LPATHS) -o $@ $^ $(LFLAGS) -no-pie
endif

%.o: %.cc *.hh Makefile 
	$(CC) $(CFLAGS) $(CPATHS) -c $< -o $@ -no-pie
